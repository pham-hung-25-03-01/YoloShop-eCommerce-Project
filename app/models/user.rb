  ...
  def self.from_omniauth(auth)
    ...
    if user.nil?
      ...
      when 'facebook'
        ...
        user = User.new(email: email, password: Devise.friendly_token[0, 20], first_name: auth.info.first_name, last_name: auth.info.last_name, avatar_url: auth.info.image, provider: auth.provider, uid: auth.uid)
