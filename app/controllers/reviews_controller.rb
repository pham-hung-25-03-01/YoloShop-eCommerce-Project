  ...
  def favorite
    begin
      if user_signed_in?
        ...
        count_favorite = Review.where(
          user_id: current_user.id,
          is_favored: true
        ).count
        render json: {
          html: render_to_string(
            partial: 'layouts/partials/favorite_item',
            locals: {
              item: review
            }
          ),
  ...
  def rating
    begin
      if user_signed_in?
        ...
        product.save
        render json: {
          score_rating: product.score_rating,
          number_of_rates: product.number_of_rates,
          user_id: current_user.id,
          is_signed_in: true
        }
      else
