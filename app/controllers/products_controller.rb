class ProductsController < ApplicationController
  ...
  def show
    begin
      ...
      all_comments = Comment.where(
        product_id: @product.id, is_actived: true
      ).order(
        created_at: :DESC
      )
  ...
  def get_quantity_in_stock
    begin
      @inventory = Inventory.find_by(
        ...
        color_url: params[:color],
        size: params[:size]
      )
      render json: {
        quantity_of_inventory: @inventory.nil? ? 0 : @inventory.quantity_of_inventory
      }
    rescue StandardError => e
      p e.message
      p e.backtrace
  ...
  def do_filter(type_filter, id, option, search_keyword)
    ...
    case option
    when 'name'
      all_products = all_products.order(
        product_name: :ASC
      )
    when 'newest'
      all_products = all_products.order(
    ...
    when 'price'
      all_products = all_products.order(
        Arel.sql('(products.sell_price * (1 - products.product_discount / 100)) ASC')
      )
    end
    all_products = all_products.where(
      "LOWER(product_name) LIKE ?", "%#{search_keyword.downcase}%"
