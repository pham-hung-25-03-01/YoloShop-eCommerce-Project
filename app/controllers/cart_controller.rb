class CartController < ApplicationController
    ...
    def update_quantity
        if user_signed_in?
            cart = Cart.find_by(inventory_id: params[:product][:inventory_id], user_id: current_user.id)
            quantity_of_inventory = Inventory.find(params[:product][:inventory_id]).quantity_of_inventory
